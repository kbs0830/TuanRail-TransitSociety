...
以台灣日本鐵道風格為主軸，結合 Solari 翻牌看板美學與歷史檔案感，介紹团‧鐵道創社故事與理念的網站。
...
- 視覺風格: 鐵道 Solari 翻牌板美學（黑底琥珀字）+ 台灣/日本路線色系統
...
- 配色: Solari 看板黑底（`#111`）搭配琥珀字（`#f0a830`），主介面保留冷灰藍紙感
- 路線色: 台鐵藍、高鐵橘、台北捷運各線色、JR 綠/藍，對應各 section 頂部色條
- 字體: 內文 Noto Serif TC，標題/數字等寬字體呈現鐵道電子板感
- 動效: Split-Flap 翻牌時鐘（HH:MM:SS）、低干擾進場動畫
- 氛圍: 日台鐵道站務員的出發看板 × 歷史檔案頁
...
```
css/
├── index.css               ⭐ 主入口（@import 順序即 layer 優先級）
├── base/
│   ├── variables.css       - CSS 變數：顏色、字體、間距、路線色（--tra-blue / --mrt-* / --jr-*）
│   ├── reset.css           - 全局重置
│   └── typography.css      - 文字排版
├── layout/
│   ├── sidebar.css         - 側邊欄基礎
...
│   ├── buttons.css         - 按鈕
│   ├── forms.css           - 表單
│   └── sidebar-rail-minimal.css - 鐵道簡約側邊欄
├── features/
│   ├── hero.css            - 英雄區與公告
│   ├── animations.css      - 動畫定義
│   ├── admin.css           - 後台管理
│   ├── home-rebuild.css    - 首頁整體版型（layer 6）
│   └── rail-elements.css   - 鐵道視覺元素：翻牌時鐘/站板/月台號/路線圖（layer 7，最高優先）
└── responsive/
...
└── modules/
    ...
    ├── scroll.js         - 滾動、時鐘、加載器
    ├── episode.js        - 章節管理
    ├── train-board.js    - 翻牌顯示器（出發看板資料）
    ├── station-clock.js  - Split-Flap 翻牌時鐘（HH:MM:SS）
    └── home-controller.js - 首頁初始化控制器
```
...
| CSS 文件數 | 1 | 15 | +1400% |
| JS 文件數 | 1 | 10 | +900% |
| 單個文件行數 | ~1700 (CSS) | ~100-450 (各模組) | ✅ 更易維護 |
| 鐵道路線色變數 | 0 | 10 | ✅ 統一管理 |
| 維護難度 | 高 | 低 | ✅ |
| 可擴展性 | 困難 | 容易 | ✅ |
